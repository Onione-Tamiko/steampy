...
##            return  # Session is alive, no need to login again
    ...
    def get_wallet_balance(self, convert_to_decimal: bool = True, on_hold: bool = False) -> Union[str, Decimal]:
        ...
        if convert_to_decimal:
            return Decimal(balance_dict[balance_dict_key]),balance_dict
        else:
            return balance_dict[balance_dict_key],balance_dict



    @login_required
    def fetch_history_market(self, start = 0, pagesize = 100):
        return self._session.get(f'{SteamUrl.COMMUNITY_URL}/market/myhistory/?query=&start={start}&count={pagesize}').json()

    @login_required
    def fetch_price_from_html(self, country = 'KZ', currency_id = 37,nameid = 1):
##        print(f'https://steamcommunity.com/market/itemordershistogram?language=russian&currency={currency_id}&item_nameid={nameid}')
        return self._session.get(f'https://steamcommunity.com/market/itemordershistogram?language=russian&currency={currency_id}&item_nameid={nameid}').json()

    @login_required
    def fetch_history_inventory(self,steamid, cursor_time = None, cursor_frac = None, cursor_s = None ):
        url = f'{SteamUrl.COMMUNITY_URL}/profiles/{steamid}/inventoryhistory/?ajax=1'
        if None not in [cursor_time,cursor_frac,cursor_s]:
            url = url + f'&cursor%5Btime%5D={cursor_time}&cursor%5Btime_frac%5D={cursor_frac}&cursor%5Bs%5D={cursor_s}'
